...
def save_get_payment_emails_to_three_excels(
    ...
    3) payment_source_xlsx:
         One row per Month, with:
           - totals by payment source (not categorized by person)
           - overall monthly total across all sources
    ...
    # --------------------------
    # Build payment source summary by month
    # --------------------------
    payment_source = (
        df_tx.assign(Month=df_tx["Email Date"].dt.to_period("M").astype(str))
        .pivot_table(
            index="Month",
            columns="Source",
            values="Amount",
    ...
    # ensure consistent source columns
    for col in ["Venmo", "PayPal", "Zelle"]:
        if col not in payment_source.columns:
            payment_source[col] = 0.0

    payment_source = payment_source[["Venmo", "PayPal", "Zelle"]]

    # add total column
    payment_source["Total_All"] = payment_source.sum(axis=1)

    payment_source = payment_source.reset_index()

    payment_source.to_excel(payment_source_xlsx, index=False)
    print(f"✅ Saved payment source file: {payment_source_xlsx} (months={payment_source.shape[0]})")

    return df_tx, summary, payment_source


save_get_payment_emails_to_three_excels(
    transactions_xlsx="Payments_Transactions.xlsx",
    summary_xlsx="Payments_Summary_ByPerson_ByMonth.xlsx",
    payment_source_xlsx="payment_source.xlsx",
)